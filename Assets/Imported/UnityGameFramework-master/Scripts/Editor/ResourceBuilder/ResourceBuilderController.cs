...
namespace UnityGameFramework.Editor.ResourceTools
{
    public sealed partial class ResourceBuilderController
    {
        ...
        private bool BuildResources(Platform platform, AssetBundleBuild[] assetBundleBuildDatas, BuildAssetBundleOptions buildAssetBundleOptions, ResourceData[] assetBundleResourceDatas, ResourceData[] binaryResourceDatas)
        {
            ...
            var parameters = new BundleBuildParameters(buildTarget, BuildPipeline.GetBuildTargetGroup(buildTarget), workingPath);
            //parameters.UseCache = false;
            //parameters.WriteLinkXML = true;

            switch (AssetBundleCompression)
            {
                case AssetBundleCompressionType.Uncompressed:
                    {
                ...
                case AssetBundleCompressionType.LZ4:
                    {
                        parameters.BundleCompression = parameters.Target == BuildTarget.WebGL ? UnityEngine.BuildCompression.LZ4Runtime : UnityEngine.BuildCompression.LZ4;
                    }
                    break;
            ...
            IBundleBuildContent content = new BundleBuildContent(assetBundleBuildDatas);
            ...
            {
                Debug.LogErrorFormat("Build asset bundles for '{0}' failure, returnCode: {1}", platformName, exitCode.ToString());

                m_BuildReport.LogError("Build asset bundles for '{0}' failure, returnCode: {1}", platformName, exitCode.ToString());
            ...
            if (m_BuildEventHandler != null)
            {
                ...
                m_BuildEventHandler.OnBuildAssetBundlesComplete(platform, workingPath, OutputPackageSelected, outputPackagePath, OutputFullSelected, outputFullPath, OutputPackedSelected, outputPackedPath, /*AssetBundleManifest*/null);
            }
