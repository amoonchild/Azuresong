...
namespace Sars.Editor.DataTableTools
{
    public sealed class DataTableGenerator
    {
        ...
        public static string GetDataTablePath(string dataTableName)
        {
            ...
            return Utility.Path.GetRegularPath(Path.Combine(dataTablePath, dataTableName + ".txt"));
        }
        ...
        {
            string dataTablePath = Type.GetConfigurationPath<DataTableOutputFilePathAttribute>();
            if (string.IsNullOrEmpty(dataTablePath))
            {
                return;
            }
            ...
            {
                Debug.LogFormat("Generate DataTable Data File '{0}' success.", dataTableName);
            }
        }

        public static void GenerateCodeFile(DataTableProcessor dataTableProcessor, string dataTableName)
        {
            string csharpCodeTempPath = Type.GetConfigurationPath<DataTableCSharpCodeTemplatePathAttribute>() ?? "Assets/Framework/Scripts/Editor/Settings";
            if (string.IsNullOrEmpty(csharpCodeTempPath))
            {
                return;
            }

            string csharpCodePath = Type.GetConfigurationPath<DataTableCSharpCodePathAttribute>();
            ...
            {
                Debug.LogFormat("Generate DataTable Code File '{0}' success.", dataTableName);
            }
        }
        ...
        private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
        {
            ...
            //codeContent.Replace("__DATA_TABLE_NAME_SPACE__", "SarsFramework.Runtime");
