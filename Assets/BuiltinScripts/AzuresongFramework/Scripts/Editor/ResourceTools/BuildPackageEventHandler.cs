...
//  Description：BuildPackageEventHandler
//------------------------------------------------------------
using GameFramework;
using System.IO;
using UnityEditor;
...
namespace Sars.Editor
{
    public class BuildPackageEventHandler : IBuildEventHandler
    {
        ...
        {
            if (!outputPackageSelected || !isSuccess)
            {
                return;
            }
            ...
            {
                fileNames = Directory.GetFiles(streamingAssetsPath, "*", SearchOption.AllDirectories);
            ...
            Debug.Log("Copy package resources success.");
        }
    }
}