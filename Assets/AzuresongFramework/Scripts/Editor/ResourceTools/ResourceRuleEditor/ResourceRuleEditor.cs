...
namespace Azuresong.Editor
{
    ...
    {
        public static readonly string ConfigurationPath = "Assets/AzuresongFramework/Scripts/Editor/Settings/ResourceRuleEditorData.asset";
        ...
        private static string m_SourceAssetExceptTypeFilter = "t:Script";
        private static string[] m_SourceAssetExceptTypeFilterGUIDArray;

        private static string m_SourceAssetExceptLabelFilter = "l:ResourceExclusive";
        ...
        private void OnEnable()
        {
            ...
            m_ResourceCollection = null;
            m_SourceAssetExceptTypeFilterGUIDArray = null;
            m_SourceAssetExceptLabelFilterGUIDArray = null;
        }
        ...
        {
            m_Configuration = LoadAssetAtPath<ResourceRuleEditorData>(ConfigurationPath);
            if (m_Configuration == null)
            {
        ...
        {
            if (LoadAssetAtPath<ResourceRuleEditorData>(ConfigurationPath) == null)
            {
                AssetDatabase.CreateAsset(m_Configuration, ConfigurationPath);
            }
            else
            {
                EditorUtility.SetDirty(m_Configuration);
        ...
        public static bool RefreshResourceCollection()
        {
            if (m_Configuration == null)
            {
                Load();
            ...
            {
                Debug.LogError("Refresh ResourceCollection.xml fail");
                return false;
            }
        }

        private static GFResource[] GetResources()
        {
                        ...
                        case ResourceFilterType.Children:
                            {
                                ...
                                for (int i = 0; i < patterns.Length; i++)
                                {
                                    ...
                                    foreach (FileInfo file in assetFiles)
                                    {
                                        ...
                                        string assetGUID = GetGUID(assetName);
        ...
        private static string GetGUID(string assetName)
        {
            string guid = string.Empty;
            if (assetName.Contains("PackageCache"))
            {
                string metaPath = Application.dataPath.Replace("/Assets", "") + "/Library" + GameFramework.Utility.Path.GetRegularPath(assetName) + ".meta";
                ...
                using (FileStream fs = new FileStream(metaPath, FileMode.Open, FileAccess.Read, FileShare.None))
                {
